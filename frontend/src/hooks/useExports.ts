...
export const useExports = () => {
  ...
  const fetchJobs = useCallback(async (skip = 0, limit = 20) => {
    ...
    setError(null);
    try {
      const response = await api.get(`/api/exports?skip=${skip}&limit=${limit}`);
  ...
  const getJobStatus = useCallback(async (jobId: number) => {
    try {
      const response = await api.get(`/api/exports/${jobId}`);
      const data: ExportJob = await response.json();
  ...
  const cancelJob = useCallback(
    async (jobId: number) => {
      setError(null);
      try {
        const response = await api.post(`/api/exports/${jobId}/cancel`, {});
        const data: ExportJob = await response.json();
  ...
  const deleteJob = useCallback(
    async (jobId: number) => {
      setError(null);
      try {
        const response = await api.delete(`/api/exports/${jobId}`);
